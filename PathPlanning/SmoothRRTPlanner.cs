...
namespace RFC.PathPlanning
{
    public class SmoothRRTPlanner
    {
        ...
        //Constants-------------------------------------
        double TIME_STEP = 0.12; //Time step in secs, with velocity determines RRT extension length
        double ROBOT_VELOCITY = 1.0; //Assume our robot can move this fast
        double MAX_ACCEL_PER_STEP = 0.48; //And that it can accelerate this fast.
        double MAX_OBSERVABLE_VELOCITY = 0.4; //Pretend that the current robot is moving at most this fast

        //RRT parameters
        int MAX_TREE_SIZE = 200; //Max nodes in tree before we give up
        int MAX_PATH_TRIES = 70; //Max number of attempts to extend paths before we give up
        double CLOSE_ENOUGH_TO_GOAL = 0.001; //We're completely done when we get this close to the goal.
        double DIST_FOR_SUCCESS = 1.2; //We're done for now when we get this much closer to the goal than we are

        double DODGE_OBS_DIST = 1.6;

        //Path extension
        double EXTRA_EXTENSION_ROTATE_ANGLE = 2.0;

        //Motion extrapolation
        double ROBOT_MAX_TIME_EXTRAPOLATED = 0.7; //Extrapolate other robots' movements up to this amount of seconds.

        //Collisions
        double ROBOT_RADIUS = Constants.Basic.ROBOT_RADIUS; //Radius of single robot
        double ROBOT_AVOID_DIST = 0.185;  //Avoid robot distance
        double ROBOT_FAR_AVOID_DIST = 0.21; //Avoid robot distance when not close to goal
        double ROBOT_FAR_DIST = 0.35; //What is considered "far", for the purposes of robot avoidance? 

        //Scoring
        double DIST_FROM_GOAL_SCORE = 0; //Penalty per m of distance from goal.
        double EXCESS_LEN_SCORE = 60; //Penalty for each m of path length >= the straightline distance
        double PER_BEND_SCORE = 1; //Bonus/Penalty per bend in the path based on bend sharpness
        double VELOCITY_AGREEMENT_SCORE = 30; //Bonus for agreeing with current velocity, per m/s velocity
        double OLDPATH_AGREEMENT_SCORE = 200; //Bonus for agreeing with the old path, per m/s velocity
        double OLDPATH_AGREEMENT_DIST = 0.5; //Score nothing for points that differ by this many meters from the old path
        double MIN_WEIGHT_VEL = 0.1; // min vel for weight; if less than this velocity, take velocity equal to it

        int NUM_PATHS_TO_SCORE = 9; //How many paths do we generate and score?

        Rectangle LEGAL_RECTANGLE;
        int goalie_id;
        ...
        public SmoothRRTPlanner(bool includeCurStateInPath, int numberRobots, int goalie_id)
        {
            ...
            LEGAL_RECTANGLE = ExpandRectangle(Constants.FieldPts.EXTENDED_FIELD_RECT, -ROBOT_RADIUS); //The basic rectangle determining what points in the field are legal to move to
            ...
            RobotPath oldPath, DefenseAreaAvoid leftAvoid, DefenseAreaAvoid rightAvoid)
        {
            ...
            LineSegment topWall = new LineSegment(Constants.FieldPts.RIGHT_GOAL_BOX.TL, Constants.FieldPts.RIGHT_GOAL_BOX.TR);
            LineSegment rightWall = new LineSegment(Constants.FieldPts.RIGHT_GOAL_BOX.TR, Constants.FieldPts.RIGHT_GOAL_BOX.BR);
            LineSegment bottomWall = new LineSegment(Constants.FieldPts.RIGHT_GOAL_BOX.BL, Constants.FieldPts.RIGHT_GOAL_BOX.BR);
            obstacles.Add(ExpandLineSeg(topWall, ROBOT_RADIUS));
            obstacles.Add(ExpandLineSeg(rightWall, ROBOT_RADIUS));
            obstacles.Add(ExpandLineSeg(bottomWall, ROBOT_RADIUS));
