...
namespace RFC.PathPlanning
{
    public static class Avoider
    {
        ...
        public static bool isValid(Point2 target, bool is_goalie)
        {
            if (!in_bounds(target))
                return false;
        ...
        // kickin and kickoff
        public static RobotInfo avoid(RobotInfo target, Point2 obstacle, double radius)
        {
            ...
            List<Point2> intersects = get_intersections(obstacle, radius);
            double mindist = 100000;
            Point2 minpt = target.Position;
            foreach (Point2 pt in intersects)
            {
        ...
        public static RobotInfo avoid(RobotInfo target, Point2 obst1, double rad1, Point2 obst2, double rad2,
            Point2 obst3, double rad3, Point2 obst4, double rad4)
        {
            RobotInfo lastResult = new RobotInfo(new Point2(-10, -10), 0, target.Team, 0);
            RobotInfo result = new RobotInfo(target);
        ...
        private static bool in_bounds(Point2 Position)
        {
        ...
        private static List<Point2> get_intersections(Point2 obst, double radius)
        {
            /*
             * x^2 + y^2 = r^2
             * x = +-sqrt(r^2 - y^2)
            ...
            if (obst.Y + radius >  Constants.Field.YMAX)
            {
                ...
                pts.Add(new Point2(obst.X + d, Constants.Field.YMAX));
                pts.Add(new Point2(obst.X - d, Constants.Field.YMAX));
            }
            // bottom
            else if (obst.Y - radius < Constants.Field.YMIN)
            {
                ...
                double d = Math.Sqrt(radius * radius - y * y);
                pts.Add(new Point2(obst.X + d, Constants.Field.YMIN));
                pts.Add(new Point2(obst.X - d, Constants.Field.YMIN));
            }
            ...
            if (obst.X + radius > Constants.Field.XMAX)
            {
                ...
                double d = Math.Sqrt(radius * radius - y * y);
                pts.Add(new Point2(Constants.Field.XMAX,obst.Y + d));
                pts.Add(new Point2(Constants.Field.XMAX, obst.Y - d));
            }
            // left
            else if (obst.X - radius < Constants.Field.XMIN)
            {
                ...
                double d = Math.Sqrt(radius * radius - y * y);
                pts.Add(new Point2(Constants.Field.XMIN, obst.Y + d));
                pts.Add(new Point2(Constants.Field.XMIN, obst.Y - d));
            }

            // checking that all intersections are in bounds
            List<Point2> in_pts = new List<Point2>();
