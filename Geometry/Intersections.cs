...
namespace RFC.Geometry
{
    public static class GeomFuncs
    {
        ...
        static public bool intersects(Line a0, Arc a1)
        {
            ...
            if (intersects(a0, endpointSeg))
                return true;
            Point2 closestPoint = a0.closestPointTo(a1.Center);
        ...
        static public bool intersects(LineSegment a0, Arc a1)
        {
            ...
            Point2[] intersections = LineCircleIntersection.BothIntersections(a0.Line, a1.Circle);
        ...
        static public bool intersects(Circle a0, Arc a1)
        {
            ...
            //Just compute the intersections and test if they are within the desired angle range.
            Point2[] intersections = CircleCircleIntersection.BothIntersections(a0, a1.Circle);
            for (int i = 0; i < intersections.Length; i++)
            {
                Vector2 dir = intersections[i] - a1.Center;
                if (dir == Vector2.ZERO)
        ...
        static public bool intersects(Arc a0, Arc a1)
        {
            ...
            //Just compute the intersections and test if they are within the desired angle range.
            Point2[] intersections = CircleCircleIntersection.BothIntersections(a0.Circle, a1.Circle);
            for (int i = 0; i < intersections.Length; i++)
            {
    ...
    {
        static public Point2 intersection(Line l1, Line l2)
        {
            return LineLineIntersection.Intersection(l1, l2);
        }
        static public Point2 intersection(Line l, Circle c, int whichintersection)
        {
            return LineCircleIntersection.Intersection(l, c, whichintersection);
        }
        static public Point2 intersection(Circle c1, Circle c2, int whichintersection)
        {
    ...
    {
        static public Point2[] BothIntersections(Circle c0, Circle c1)
        {
            return GetPoints(c0, c1);
        }

        static public Point2 Intersection(Circle c0, Circle c1, int whichintersection)
        {
            Point2[] bothpoints = GetPoints(c0, c1);
        ...
        /// </summary>
        static public int WhichIntersection(Circle c0, Circle c1, Point2 p)
        {
        ...
        static private Point2[] GetPoints(Circle c0, Circle c1)
        {
            Point2 p0 = c0.Center;
            Point2 p1 = c1.Center;
            ...
            Point2[] bothpoints = new Point2[2];
            double newx = p0.X + (p1.X - p0.X) * a / d;
            double newy = p0.Y + (p1.Y - p0.Y) * a / d;
            double h = Math.Sqrt(r0 * r0 - a * a);
            double dx = h * (p1.Y - p0.Y) / d;
            double dy = h * (p0.X - p1.X) / d;
            bothpoints[0] = new Point2(newx + dx, newy + dy);
            bothpoints[1] = new Point2(newx - dx, newy - dy);
            return bothpoints;
        }
        static private int anglesign(Point2 p1, Point2 p2, Point2 p3)
        {
    ...
    {
        static public Point2[] BothIntersections(Line line, Circle circle)
        {
            return getPoints(line, circle);
        }

        static public Point2 Intersection(Line line, Circle circle, int whichintersection)
        {
            double[] dists = new double[2];
            Point2[] points = getPoints(line, circle);
            if(points.Length == 0)
                throw new NoIntersectionException("no intersection!");
        ...
        static public int WhichIntersection(Line line, Circle circle, Point2 p)
        {
            Point2[] points = getPoints(line, circle);
            if (points.Length == 0)
                throw new NoIntersectionException("no intersection!");
        ...
        static private double distAlongLine(Point2 p, Line line)
        {
        ...
        static private Point2[] getPoints(Line line, Circle circle)
        {
            Point2 center = circle.Center;
            line = line - (center - Point2.ORIGIN);
            ...
            rtnpoints[0] = center + new Vector2((x0 - ddx) / drs, (y0 - ddy) / drs);
            rtnpoints[1] = center + new Vector2((x0 + ddx) / drs, (y0 + ddy) / drs);
    ...
    static public class LineCircleIntersectionDeprecated
    {
        static public Point2 Intersection(Line line, Circle circle, int whichintersection)
        {
            Point2[] linepoints = {line.P0,line.P1};
            if (whichintersection == 1)
            {
                Point2 temp = linepoints[0];
        ...
        private static double distalongline(Point2 p, Point2[] linepoints)
        {
        ...
        {
            Point2 center = circle.Center;
            Vector2[] points = { line.P0 - center, line.P1 - center };
    ...
    {
        static public Point2 Intersection(Line line0, Line line1)
        {
            Point2[] l0 = { line0.P0, line0.P1 };
            Point2[] l1 = { line1.P0, line1.P1 };
